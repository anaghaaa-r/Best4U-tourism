...
class DestinationPackageController extends Controller
{
    ...
    public function view($id)
    {
        ...
        return view('admin.destination-package', [
            'model' => $model,
            'packages' => $packages
        ]);
    }
    ...
    public function createPackage(Request $request, $id)
    {
        ...
        try {
            ...
            if ($thumbnailDimensions[0] !== 370 || $thumbnailDimensions[1] !== 240) {
                return response()->json([
                    'status' => false,
            ...
            $thumbnailPath = 'uploads/destinations/packages/' . $filename;
            ...
            if($request->hasFile('image'))
            {
                ...
                $imagePath = 'uploads/destinations/packages/' . $filename;
            ...
            $destination->package()->create($packageDetails);
    ...
    // edit package
    public function editPackage(Request $request, $destinationId, $packageId)
    {
        try {
            $destination = Destination::findOrFail($destinationId);

            $package = $destination->package()->findOrFail($packageId);
    ...
    // delete package
    public function deletePackage($destinationId, $packageId)
    {
        try {
            $destination = Destination::findOrFail($destinationId);
